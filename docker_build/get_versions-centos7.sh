...
RES=`docker run --rm -t centos:centos7 bash -c 'echo -e "[metwork]\nname=Metwork Continuous Integration Branch '${BRANCH_NAME}'\nbaseurl=ftp://synapse.meteo.fr/pub/metwork/releases/rpms/continuous_integration/centos7/'${BRANCH_NAME}'\ngpgcheck=0\nenabled=1\nmetadata_expire=0\n" >/etc/yum.repos.d/metwork.repo ; yum --disablerepo=* --enablerepo=metwork -q list available '${PACKAGE} |grep "^${PACKAGE}"`
if test $? -ne 0; then
    BRANCH_NAME=master
    RES=`docker run --rm -t centos:centos7 bash -c 'echo -e "[metwork]\nname=Metwork Continuous Integration Branch master\nbaseurl=ftp://synapse.meteo.fr/pub/metwork/releases/rpms/continuous_integration/centos7/master\ngpgcheck=0\nenabled=1\nmetadata_expire=0\n" >/etc/yum.repos.d/metwork.repo ; yum --disablerepo=* --enablerepo=metwork -q list available '${PACKAGE} |grep "^${PACKAGE}"`
    if test $? -ne 0; then
